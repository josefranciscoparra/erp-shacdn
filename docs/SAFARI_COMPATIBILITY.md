...
```tsx
// Component
<div className="wizard-action-bar bg-background border-t">{/* Contenido del footer */}</div>
```
...
```tsx
<div className="bg-muted-foreground/30 h-1 w-full" />
```
...
<div className="flex min-h-screen flex-col gap-4 md:gap-6">
  ...
  <div className="flex-1">{/* Formularios */}</div>
...
<div className="sticky bottom-0 z-50 mt-auto">
  <div className="wizard-action-bar bg-background border-t">{/* Botones */}</div>
</div>
```
