    ...
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[1800px]">
          {/* Header: Días del mes */}
          <div className="sticky top-0 z-10 mb-2 grid gap-1 bg-background pb-2" style={{ gridTemplateColumns: `180px repeat(${monthDays.length}, 50px)` }}>
            {/* Columna de zonas */}
              ...
              <div key={costCenterId} className="mb-6">
                ...
                <div className="space-y-2">
                  {zonesInCenter.map((zone) => (
                    <div key={zone.id} className="grid gap-1" style={{ gridTemplateColumns: `180px repeat(${monthDays.length}, 50px)` }}>
                      {/* Columna: Nombre de la zona */}
                      <div className="flex flex-col justify-center rounded-lg border bg-card px-3 py-2">
                        <p className="text-sm font-semibold">{zone.name}</p>
                        <p className="text-muted-foreground text-[10px]">
                          Req: {zone.requiredCoverage.morning}/{zone.requiredCoverage.afternoon}/{zone.requiredCoverage.night}
                        </p>
                      </div>
