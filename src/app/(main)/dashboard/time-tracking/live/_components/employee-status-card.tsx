...
export function EmployeeStatusCard({ employee }: EmployeeStatusCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Hora de entrada si está trabajando, o horas trabajadas si no */}
      {employee.status === "CLOCKED_IN" && employee.lastAction ? (
        <div className="text-muted-foreground shrink-0 text-xs font-medium">
          Entrada: {format(new Date(employee.lastAction), "HH:mm", { locale: es })}
        </div>
      ) : (
