...
export function EditPeriodDialog({ period, templateType, variant = "ghost", size = "sm" }: EditPeriodDialogProps) {
  ...
  async function onSubmit(data: FormValues) {
    ...
    try {
      const result = await updateSchedulePeriod(period.id, {
        name: data.name,
      ...
      if (result.success) {
        toast.success("Período actualizado", {
          description: `El período "${data.name}" se ha actualizado correctamente`,
        });
        setOpen(false);
        router.refresh();
      } else {
      ...
      toast.error("Error al actualizar período", {
        description: "Ha ocurrido un error al actualizar el período",
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>Modifica los datos del período de horario</DialogDescription>
        </DialogHeader>
