...
function PtoDocuments({ item }: { item: PendingApprovalItem }) {
  const documents = item.details?.documents as Array<{
    id: string;
    fileName: string;
    fileSize: number;
    mimeType: string;
  ...
  return (
    <div className="mt-4 space-y-3">
      ...
      {requiresDocument && documentsCount === 0 && (
        <div className="bg-amber-50 text-amber-800 dark:bg-amber-950/30 dark:text-amber-400 flex items-start gap-2 rounded-md p-3 text-sm">
    ...
    <div className="space-y-6">
      ...
      {reason && (
        <div className="bg-muted/30 rounded-md p-3 text-sm">
          <span className="text-muted-foreground block text-xs font-medium mb-1">Motivo de la solicitud</span>
          <p className="italic text-foreground/90">&quot;{reason}&quot;</p>
        </div>
      )}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <p className="font-medium text-lg">{clockIn ? format(clockIn, "HH:mm", { locale: es }) : "-"}</p>
        </div>
        <div className="space-y-1">
          <span className="text-muted-foreground flex items-center gap-1 text-xs">
            <Clock className="h-3.5 w-3.5" /> Salida
          </span>
          <p className="font-medium text-lg">{clockOut ? format(clockOut, "HH:mm", { locale: es }) : "-"}</p>
        </div>
      </div>

      <div className="bg-muted/30 rounded-md p-3 text-sm">
        <span className="text-muted-foreground block text-xs font-medium mb-1">Motivo de la corrección</span>
        <p className="italic text-foreground/90">{reason ?? "Sin motivo especificado"}</p>
      </div>
    </div>
  );
}
...
function ExpenseDetails({ item }: { item: PendingApprovalItem }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {notes && (
        <div className="bg-muted/30 rounded-md p-3 text-sm">
          <span className="text-muted-foreground block text-xs font-medium mb-1">Notas del empleado</span>
          <p className="italic text-foreground/90">&quot;{notes}&quot;</p>
        </div>
      )}

      {downloadUrl ? (
        <div className="border-t pt-4">
          <span className="text-muted-foreground flex items-center gap-2 text-sm font-medium mb-3">
            <Paperclip className="h-4 w-4" /> Comprobante Adjunto
          </span>
          <div className="flex gap-2">
            <Button
                variant="outline"
                className="flex-1 justify-center h-auto py-2"
                onClick={() => {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Textarea
                id="comments"
                placeholder={action === "approve" ? "Añadir nota para el empleado..." : "Explica el motivo del rechazo..."}
                value={comments}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] p-0 flex flex-col gap-0">
        ...
        <DialogFooter className="p-6 border-t flex flex-col-reverse sm:flex-row gap-2 sm:justify-end">
          {isReadOnly ? (
