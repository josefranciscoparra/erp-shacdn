...
export async function GET(
  ...
  try {
    ...
    try {
      const fileBlob = await documentStorageService.downloadDocument(
        document.storageUrl
      );
      ...
      headers.set(
        "ETag",
        `"${document.id}-${document.updatedAt.getTime()}"`
      );
      ...
      return NextResponse.json(
        {
          error:
            "No se pudo acceder al archivo. Puede que haya sido eliminado.",
        },
        { status: 404 }
      );
    }
