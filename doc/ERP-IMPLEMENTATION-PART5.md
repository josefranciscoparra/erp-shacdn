...
export const payrollRouter = createTRPCRouter({
  ...
  getPayrollMapping: protectedProcedure.use(withPermission("PAYROLL_CONFIGURE")).query(async ({ ctx }) => {
    const org = await ctx.prisma.organization.findUnique({
      where: { id: ctx.orgId },
    });

    return (
      org?.settings?.payrollMapping || {
        concepts: {
          REGULAR_HOURS: "001",
          OVERTIME_HOURS: "002",
          NIGHT_HOURS: "003",
...
export async function generatePayrollExport(params: PayrollExportParams): Promise<{
  fileName: string;
  url: string;
  records: any[];
  format: string;
  ...
  for (const employee of employees) {
    ...
    const absenceDays = empAbsences.reduce(
      (acc, absence) => {
...
export function validatePayrollPeriod(startDate: Date, endDate: Date): { valid: boolean; error?: string } {
